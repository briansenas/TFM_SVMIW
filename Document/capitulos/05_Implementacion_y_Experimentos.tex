...
\par
Finalmente, se detallan los errores absolutos obtenidos con cada uno de los métodos, así como una comparación frente a un método de
...
No obstante, el método de calibración manual depende de mediciones rigurosas de la escena, así como del uso
...
sobre el conjunto de datos Pano360.
Los valores obtenidos son del mismo orden de magnitud y sugieren que la
implementación reproduce razonablemente el comportamiento descrito, procesando un número inferior de ejemplos durante el entrenamiento.
En términos de número total de ejemplos de calibración vistos durante el entrenamiento, se observan 33 veces menos ejemplos que~\cite{SPEC} y 1.78 veces menos que~\cite{SVMIW}.
No obstante, la comparación no es estrictamente directa debido a las diferencias entre las particiones utilizadas.
...
presenta un comportamiento consistente en la partición utilizada.
...
\begin{table}[!ht]
  ...
  Se observa que el modelo muestra un comportamiento comparable al de referencia. Todo ello con 70K iteraciones de entrenamiento
  menos, utilizando la mitad de ejemplos por cada iteración. La partición de evaluación utilizada de KITTI-person* no es exactamente igual a la del artículo~\cite{SVMIW}.
...
las cámaras en conjunto es de 9.8 centímetros y se encuentra en el mismo orden de magnitud que el método de calibración manual de escena.
Este error es cercano al método de estimación estática de altura media, pero permitiendo la estimación de la
altura de individuos cuya estatura no es 1.75 metros en diversas escenas.
Adicionalmente, destacan los valores máximos que no llegan a sobrepasar los 33 centímetros de error, mientras que el método de calibración manual ha llegado a tener
...
El error promedio agrupado por individuo es de 7.4 centímetros, superior a los 5.8 centímetros del método
de calibración manual y los 6.8 centímetros del método de predicción estática.
La baja variabilidad antropométrica de los datos (distribución de estatura muy centrada en 1.75 metros) y
posibles problemas de aprendizaje del modelo hacen que el estimador estático presente un error medio absoluto menor.
...
la estatura de diferentes sujetos. En las Figuras~\ref{fig:AutoBest} y~\ref{fig:AutoWorst} se
observan las mejores y peores estimaciones.
...
\begin{table}[ht]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption[Tabla resumen de la comparativa final, con errores expresados en metros.]{
  Tabla resumen de la comparativa final, con errores expresados en metros.
  ...
  No obstante, la baja variabilidad antropométrica de los datos (distribución de estatura muy centrada en 1.75 metros), junto
  a errores de estimaciones elevados en algunas escenas, hace que el método de estimación estática
  presente un menor error medio. Aún así, dichos resultados se alcanzan de forma semi-supervisada y con pocas iteraciones,
  indicando el potencial que posee el método de cara a automatizar la estimación de la altura en entornos no controlados.
}\label{tab:Results}
\end{table}
