...
\hline
Método automático. & Método por aprendizaje profundo. & Datos de entrenamiento etiquetados con la caja delimitadora de la persona en la escena. & Evaluar un método de estimación sin referencias de la escena.\\
\hline
\end{tabularx}
...
de calibración manual de escena.
En ella, se expresa el error medio ($\mu$), la desviación típica ($\sigma$), la varianza ($\sigma^2$), la mediana,
...
Dado que los fotogramas están altamente relacionados entre sí en una misma grabación, en la Tabla~\ref{tab:ManualResultsBySubject} se
recogen los valores medios de estimación por individuo.
No se observa un patrón evidente de error asociado a una altura concreta.
...
Dado que las estimaciones presentan baja varianza debido a la naturaleza del algoritmo y de los fotogramas obtenidos de las grabaciones,
es de esperar que errores elevados en sujetos con mayor cantidad de fotogramas puedan llevar el promedio a ser más pesimista (al agrupar por fotogramas)
que al agrupar por individuos. De la misma forma, en la Tabla~\ref{tab:ManualResultsByScene}
se recogen los errores de las estimaciones agrupadas por escena.
...
los errores medios de utilizar otro modelo de referencia estático que predice siempre la altura media de 1.75 metros.
...
No obstante, los resultados indican que el método de calibración manual depende de mediciones manuales precisas de la escena, así como el uso
...
Finalmente, se añaden las últimas capas y función de pérdida para la estimación de la altura del sujeto y de la cámara.
...
mejora ligeramente frente al modelo completo evaluado tras únicamente el entrenamiento de la fase 0 (de calibración de cámara).
Esto podría deberse a que el modelo en la fase final tiene acumulado más iteraciones totales, así como
la presencia de la restricción proyectiva de la estimación de la altura que afecta a dichas predicciones.
...
\begin{table}[!ht]
  ...
  \caption[Tabla comparativa de la precisión de la estimación de los parámetros de la cámara en Pano360~\cite{SPEC}.]
  {
    ...
    con el modelo entero entrenado (entrenado solo en la fase 0)
...
El modelo parece subestimar las alturas para sujetos con altura superior a 1.75, mientras
...
\begin{figure}[!ht]
  \begin{center}
    \subfloat{\includegraphics[width=0.5\textwidth]{imagenes/chapter5/svimw_height_dist.png}}
    \subfloat{\includegraphics[width=0.5\textwidth]{imagenes/chapter5/svimw_pred_height_dist.png}}
  \end{center}
...
realizar estimaciones acertadas en entornos cerrados como una zona interior de aparcamiento.
...
limitación identificada del modelo. Un ajuste equilibrado de los pesos de la combinación
lineal de pérdidas podría ayudar a evitar este colapso. De otra forma, controlar la tasa de
...
los errores máximos observables. Se aproxima al método de estimación estática de altura media de 1.75 metros, con el beneficio de acertar en múltiples escenas, de diferentes sujetos,
la estatura exacta. En las Figuras~\ref{fig:AutoWorst} y~\ref{fig:AutoBest} se
