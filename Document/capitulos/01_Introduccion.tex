...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption[Visualización del procedimiento de metrología monocular sacada de~\cite{SVMIW}.]{
    Visualización del procedimiento de metrología monocular sacada de~\cite{SVMIW}. El objetivo es dada una imagen de entrada (izquierda), poder
  establecer las relaciones geométricas necesarias para estimar correctamente la altura de los objetos en escena (medio, derecha).
  Pudiendo incluso insertar otros objetos en la misma con la escala adecuada (derecha). }\label{fig:SVM}
\end{figure}
...
\end{center}
\caption[Ilusión óptica, la habitación de Ames.]{Ilusión óptica que ocasiona a que un mismo objeto, el muñeco de lego, se vea de diferente tamaño según la posición en la habitación
...
\begin{enumerate}
    ...
    \item \textbf{Biomecánica en deportes y salud:} Análisis postural o del movimiento en atletas y pacientes sin necesidad de equipamiento especializado.
    \item \textbf{Arquitectura y reconstrucción de escenas:} Cálculo de medidas reales de estructuras a partir de imágenes tomadas por usuarios, por ejemplo, para obtener planos aproximados de interiores sin escáneres 3D o insertar objetos en escenas.
...
Por todo ello, se considera que el presente Trabajo Fin de Máster (TFM) puede suponer una aportación relevante, tanto a nivel teórico como práctico, en el avance de técnicas robustas de metrología monocular aplicadas a contextos reales y no controlados.
...
\begin{enumerate}
    ...
    \item Validar el sistema en diferentes contextos y con variabilidad en las condiciones de iluminación, perspectiva y posición de las personas.
\end{enumerate}
...
los estudios. No obstante, dicha planificación sufrió retrasos y modificaciones significativas. Tanto por
razones técnicas, como el estado del proyecto inicial o inexistencia de parte de los conjuntos de datos del autor, como
personales. La planificación final se puede observar en la Tabla~\ref{tab:PlanificacionFinal}.
...
Por último, se ha hecho uso del servidor NGPU de la Universidad de Granada. En concreto, de la cola ``Dios'', nodo Titan.
En ese caso, se estima un coste aproximado de 2.000\officialeuro\; y, por lo tanto, un coste diario de 2.73\officialeuro.
El desglose total de los costes
...
\multicolumn{1}{|l|}{Salario} & \multicolumn{1}{l|}{5.526,40\officialeuro} \\ \hline
...
\multicolumn{1}{|l|}{Servidor GPU} & \multicolumn{1}{l|}{2013,90\officialeuro} \\ \hline
\multicolumn{1}{|l|}{Servidor NGPU} & \multicolumn{1}{l|}{401,31\officialeuro} \\ \hline
...
\multicolumn{1}{|r|}{\cellcolor[HTML]{FFCB2F}{\textbf{Total}}} & \multicolumn{1}{l|}{ 9.007,56 \officialeuro} \\ \hline
\textbf{} &
\end{tabular}
