...
del entrenamiento en precisión mixta, AMP).
...
la posibilidad de redefinir o modificar flujos existentes por medio de la sobre-carga de
funciones y métodos, sin tener que modificar la lógica principal del código. Un ejemplo de ello
es la sobre-carga del método \lstinline{visualize_training} de la meta-arquitectura que permite
guardar inferencias durante el entrenamiento para observar si el modelo está aprendiendo correctamente.
En la Figura~\ref{fig:VisualizeTraining},
se observan las predicciones del módulo de estimación
de parámetros de la cámara durante el entrenamiento.
\par
...
\end{enumerate}
Dichas ejecuciones se pueden concatenar haciendo uso del conector de ejecuciones exítosas de Linux con ``\lstinline{&&}''. De
...
mayor probabilidad a los tipos más comunes (por ejemplo, al ratio 4:3 se asigna un 66\%).
